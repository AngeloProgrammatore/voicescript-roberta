...
export async function POST(req: NextRequest) {
  try {
    ...
    const prompt = "Usando il testo seguente (trascritto da un messaggio vocale), crea uno script video della durata di " + durationLabel + " in lingua " + langName + ".\n\n" +
      ...
      "- TITOLO: sulla primissima riga scrivi SOLO un titolo breve e accattivante per il video (senza virgolette, senza prefissi). Poi vai a capo due volte e inizia lo script.\n" +
    ...
    // Find where the script body starts (skip empty lines after title)
    let bodyStart = 1;
